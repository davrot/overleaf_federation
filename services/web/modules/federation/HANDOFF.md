...
| **V1-SSO** | **eduGAIN/DFN-AAI (SAML) + GEANT AAI (OIDC) SSO interop (plan/10): P1a framework port, P1b N-provider, P1c per-provider attrFilter, R1 synthetic-email JIT** | **SHIPPED** (P1a `0afb9048be`; P1b `90baca21f7`/`157eea5ec5`; P1c `8a99c05e79`; **R1 — SESSION 16**). Phases 0/2/3/4 remain **BLOCKED** on external Shibboleth-SP / DFN test IdP / GEANT Sandbox infra (plan/10). **Plan 11 SHIPPED (S17 shipped — commit `25f412f765`; recon+implement this session)**: `/saml/meta` SP registration metadata (own entityID/org/contacts + signing seam + admin UI tab + spMetadata masking, 13/13 tests) — plan 11 app-side complete; GEANT/DFN registration form = operator step (plan 11 §3) |
...
**S17 IMPLEMENTATION (DONE — SHIPPED commit `25f412f765`; code + pug + 13/13 tests + scoped lint, committed+pushed):**
