...
| **V1-SSO** | **eduGAIN/DFN-AAI (SAML) + GEANT AAI (OIDC) SSO interop (plan/10): P1a framework port, P1b N-provider, P1c per-provider attrFilter, R1 synthetic-email JIT** | **SHIPPED** (P1a `0afb9048be`; P1b `90baca21f7`/`157eea5ec5`; P1c `8a99c05e79`; **R1 — SESSION 16**). Phases 0/2/3/4 remain **BLOCKED** on external Shibboleth-SP / DFN test IdP / GEANT Sandbox infra (plan/10). **Plan 11 (SESSION 17): IMPL IN PROGRESS, UNCOMMITTED** — `/saml/meta` rewrite (own entityID/org/contacts/signing seam) + `ssoConfigs.spMetadata` masking/restore in SSO admin DONE (working tree); pug tab + `spMetadata.test.mjs` + lint/test/commit NEXT (see S17 entry) |
...
**S17 IMPLEMENTATION (DONE — code + pug + 13/13 tests green + scoped lint clean; commit+push this step):**
- DONE `modules/authentication/saml/app/src/SAMLAuthenticationController.mjs`: `getSPMetadata` rewritten — `isSAMLEnabled()`/`EXTERNAL_AUTH` guard (404 `SAML is not enabled` when off — fixes BUG 5), reads `spMetadata` from `ssoConfigs`, delegated to exported `buildSPMetadataXml(sp, siteUrl)` + `spFilename(sp, siteUrl)` (pure, exported for admin reuse + unit tests). Params: `issuer=spEntityId||<siteOrigin>/saml` (fixes BUG 1), `callbackUrl`/`logoutCallbackUrl` from `Settings.siteUrl`, `identifierFormat` persistent default, `metadataOrganization` (xmlbuilder shape from `sp.organization`), `metadataContactPerson` (contactType+email), v5 signing seam **IFF `privateKey` AND `publicCert`** → `signMetadata + private key + publicCerts:[cert] + signatureAlgorithm:'sha256'` (BUG 3 — empirically verified: key ALONE throws "Missing publicCert", key pair ⇒ signed XML with KeyDescriptors + `<Signature>`; unsigned ⇒ 0 KeyDescriptor). `Content-Type: application/saml-metadata+xml; charset=utf-8` + nosniff (fixes BUG 4/5). Strategy/IdP-direction path untouched.
- DONE `modules/authentication/admin/app/src/SSOAdminController.mjs`: `_maskConfig` masks `spMetadata.privateKey`+`publicCert` → `••••••••`; `_sanitizeConfig` restores both on save when sentinel present. NO new routes (`spMetadata` transparently rides `GET`/`POST /admin/sso/config` — the `spMetadata` subdoc is a raw findOne, no migration).
- FIXED latent SyntaxError found via `node --check`: `_sanitizeConfig`'s `.map()` closure was missing `return p })` after the S17 edit (caught by `node --check`, restored; both files now `node --check` clean).
- DONE `plan/11` §2.2: final model = `privateKey` + `publicCert` (both masked); signing IFF both present; route simplification (no new routes).
- DONE `modules/authentication/admin/app/views/sso-admin.pug` — 4th tab "SAML Metadata" (`#tab-saml-meta` nav): SP Entity ID, Organization {name/displayName/url}, Contact Persons (one-per-line `type: email` textarea), Private Key + Public Cert (textarea with `(unchanged) ••••••••` placeholders when stored values exist), `/saml/meta` preview link. Wired into `loadConfig()` + `gatherConfig()`→POST (sentinel preserved when fields empty; no new routes). Pug compiles (standalone `pug.compile` + inline-JS `node --check` clean).
- DONE `modules/authentication/test/unit/spMetadata.test.mjs` (13/13): default `<siteOrigin>/saml` entityID + explicit override; own-id-not-IdP-issuer; Organization+ContactPerson (`xml:lang`); contact default-type + no-email drop; unsigned default/key-only/cert-only (BUG 3 matrix: key-only THROWS in v5, cert-only ⇒ 0 KeyDescriptor, pair ⇒ `<KeyDescriptor use="signing">`+`<Signature>` with cert body, key NOT embedded); `spFilename` slug fallback (non-URL entity IDs); GET route 404 `SAML is not enabled` (BUG 5) + saml-metadata Content-Type + nosniff; admin `getConfig` mask (pair masked, spEntityId/org/contacts intact); `saveConfig` sentinel restore AND sentinel-with-nothing-stored ⇒ **clear** (no sentinel persisted).
- DONE scoped lint: SAML controller + test file pass `eslint --max-warnings 0` (repo `import/no-extraneous-dependencies` forbids implicit deps → import via `@node-saml/passport-saml`, already a declared direct dep that `__exportStar`s the v5 standalone fn — no new package.json entry). `SSOAdminController.mjs`: 3 pre-existing errors at 258/326/329 (identical at HEAD: `ldapjs` + dynamic `SAMLModuleManager`/`OIDCModuleManager` imports) — NOT fixed (pre-existing debt).
...
- NOT YET (next pass, then close TODO-223faa6a) :
  1. `authentication/admin/app/views/sso-admin.pug` — 4th tab "SAML SP Metadata" (spEntityId, org name/displayName/url, contacts rows, privateKey/publicCert masked fields) wired into `gatherConfig()`→POST body + `loadConfig()` + `_getConfig` GET.
  2. `authentication/test/unit/spMetadata.test.mjs` — own entityID (incl. default `<siteOrigin>/saml`), org+contacts present, signed-when-key-pair (KeyDescriptor), unsigned fallback, SAML-disabled 404, mask/restore sentinel round-trip.
  3. `yarn lint` (scoped) + full vitest + HANDOFF close + commit + push (goal 3dad1c9e SSO track).
